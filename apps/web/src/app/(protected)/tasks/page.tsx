...
export default function TasksPage() {
  ...
  const handleCalendarCreate = async (input: {
    ...
    try {
      ...
      const optimisticCreatedTask: TaskDoc = {
        ...
        googleSyncUpdatedAt: Timestamp.now(),
        archived: false,
      ...
      const syncPayload = buildGoogleCalendarSyncPayload({
      ...
      const syncResult = await createGoogleCalendarEvent(syncPayload);
      ...
      if (!syncResult.ok) {
        ...
        showActionFeedback(syncResult.message || GOOGLE_SYNC_FAILED_MESSAGE);
        return;
      }
      ...
      if (syncResult.reconciled) {
        showActionFeedback("Lien Google Calendar réconcilié automatiquement.");
      }
    } catch (error) {
  ...
  const handleCalendarUpdate = async (input: {
    ...
    const optimisticTask: TaskDoc = {
      ...
      googleSyncStatus: current.googleEventId ? "pending" : (current.googleSyncStatus ?? null),
      googleSyncError: null,
      googleSyncUpdatedAt: Timestamp.now(),
      updatedAt: Timestamp.now(),
    };
    ...
    try {
      ...
      if (current.googleEventId || current.googleSyncStatus === "error" || current.googleSyncStatus === "missing_remote") {
        const syncPayload = buildGoogleCalendarSyncPayload({
          taskId: input.taskId,
        ...
        const syncResult = await updateGoogleCalendarEvent(syncPayload);

        if (!syncResult.ok) {
          void mirrorGoogleSyncStateOnTask({
            taskId: input.taskId,
            eventId: current.googleEventId ?? null,
        ...
        void mirrorGoogleSyncStateOnTask({
          taskId: input.taskId,
          eventId: syncResult.eventId ?? current.googleEventId ?? null,
          status: "synced",
          error: null,
        }).catch(() => {});
        setOptimisticAgendaTaskById((prev) => ({
          ...
          [input.taskId]: {
            ...optimisticTask,
            googleEventId: syncResult.eventId ?? current.googleEventId ?? null,
            googleSyncStatus: "synced",
            googleSyncError: null,
            googleSyncUpdatedAt: Timestamp.now(),
          },
        ...
        if (syncResult.recreated) {
          showActionFeedback("Événement Google recréé après divergence détectée.");
        } else if (syncResult.reconciled) {
          showActionFeedback("Lien Google Calendar réparé automatiquement.");
        }
      }
    } catch (e) {
      setOptimisticAgendaTaskById((prev) => {
  ...
  const tasks = effectiveAllTasks;
  const hasTaskData = effectiveAllTasks.length > 0;
  const hasBlockingTaskReadError = Boolean(error) && !hasTaskData;
  const showInitialTaskLoadingState = loading && !hasTaskData;
  const taskReadErrorMessage = error instanceof Error ? error.message : error ? String(error) : null;
    ...
    >
      ...
      {error && (
        <div className="sn-alert sn-alert--error">
          ...
          {taskReadErrorMessage ? ` ${taskReadErrorMessage}` : ""}
        </div>
      )}

      {!showInitialTaskLoadingState && !hasBlockingTaskReadError && archiveView === "active" && mainTasks.length === 0 && (
      ...
      {!showInitialTaskLoadingState && !hasBlockingTaskReadError && archiveView === "archived" && archivedTasks.length === 0 && (
      ...
      {!showInitialTaskLoadingState && !hasBlockingTaskReadError && archiveView === "active" && viewMode === "calendar" && (
        <AgendaCalendar
          tasks={mainTasks}
          ...
          taskSourceLoading={loading}
          taskSourceError={taskReadErrorMessage}
          onOpenTask={(taskId) => {
      ...
      {!showInitialTaskLoadingState && !hasBlockingTaskReadError && archiveView === "active" && viewMode === "list" && mainTasks.length > 0 && (
        <ul className="space-y-2">
          {mainTasks.map((task) => {
            const status = (task.status as TaskStatus | undefined) ?? "todo";
            const workspaceName =
      ...
      {!showInitialTaskLoadingState && !hasBlockingTaskReadError && archiveView === "active" && AGENDA_GRID_ENABLED && viewMode === "grid" && mainTasks.length > 0 && (
      ...
      {!showInitialTaskLoadingState && !hasBlockingTaskReadError && archiveView === "active" && viewMode !== "calendar" && statusFilter === "all" && completedTasks.length > 0 && (
        <section>
