...
export function renderAgendaCalendarEventContent(arg: EventContentArg, isCompactDensity: boolean) {
  ...
  const textTone = arg.event.extendedProps.textTone === "dark" ? "dark" : "light";
  ...
  const subTextClass = textTone === "dark" ? "text-slate-900/90" : "text-white/90";
  const metaTextClass = textTone === "dark" ? "text-slate-900/95" : "text-white/95";
  ...
  if (isMonthView) {
    return (
      <div className={`px-1 py-0.5 text-[11px] leading-tight ${rootTextClass}`}>
        ...
        <div className={`text-[9px] ${subTextClass}`}>{timeLabel}</div>
      </div>
    );
  }
  ...
  return (
    <div className={`px-1 py-0.5 text-[11px] leading-tight ${rootTextClass}`}>
      ...
      <div className={`text-[10px] ${metaTextClass}`}>{timeLabel}</div>
      {compactPresentation ? (
        <div className={`mt-0.5 inline-flex flex-wrap items-center gap-1 text-[9px] ${subTextClass}`}>
          <span className="truncate">{workspaceLabel}</span>
          <span className={`rounded-full px-1.5 py-0.5 font-semibold uppercase tracking-wide ${sourceBadgeClass}`}>{sourceLabel}</span>
          {hasConflict && <span className="rounded-full bg-red-500/85 px-1.5 py-0.5 font-semibold">C P{Math.min(9, conflictScore)}</span>}
        </div>
      ) : (
        <>
          <div className={`break-words text-[10px] ${subTextClass}`}>{workspaceLabel}</div>
          <div className="mt-0.5 inline-flex flex-wrap items-center gap-1">
            <span className={`rounded-full px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wide ${sourceBadgeClass}`}>{sourceLabel}</span>
            {priority && (
              <span className={`rounded-full px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wide ${sourceBadgeClass}`}>{priority}</span>
            )}
            {hasConflict && (
